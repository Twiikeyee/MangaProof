...
受 Python 应用打包到 Android 的固有代价影响，以下几项是**已知且预期内**的，桌面端不受影响：

| 方面 | 表现 |
| --- | --- |
| 存储占用 | 安装包与安装后占用都明显大于同类原生应用（Qt 运行库与 Python 运行时一并打包） |
| 内存 | 内存档位固定为「激进」（图层像素缓存 256 MB、背景图池 68 MB），设置中不可调；大 PSD 下内存仍是主要约束 |
| 解码性能 | 桌面版用于加速 ZIP 预测压缩解码的 C 扩展不在 Android 包内，该路径退回 psd-tools 原实现，大 PSD 的预加载与像素提取慢于桌面 |
| 翻页体验 | 预加载与保留窗口收窄为 3 页（桌面 6 页），跨窗口翻页需要重建文档，会有可感知的短暂停顿 |
...
| Android | arm64-v8a / x86_64 | APK 侧载，Android 11+，强制横屏全屏；存储占用、内存与解码性能均弱于桌面，见 [Android 端的局限](#android-端的局限) |
