...
→ 下拉栏自动展开 → 直接按问题类型快捷键（`1`~`0`、`Q`~`O`…）选定类型
...
- 每类问题有**快捷键**（默认 `1`~`9`、`0`、`Q` `W` `E` `P` `T` `Y` `U` `I` `O`，可在设置中重绑定；
  `R` 留给核心快捷键「红框模式」）；
...
- **自动框选弹出的类型对话框是"键盘直通"的**：下拉栏自动展开，直接按问题类型
  快捷键（`1`~`0`、`Q`~`O`…）即为选中该类型，并自动跳到批注输入框，全程不用碰鼠标；
...
| 问题类型 | `1`~`9`、`0`、`Q` `W` `E` `P` `T` `Y` `U` `I` `O`（预制 19 类，可配置） |
