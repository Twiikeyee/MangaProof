...
**纯键盘标注流（全程不碰鼠标）**：`R` 进红框模式 → `A` 自动框选当前图层
→ 下拉栏自动展开（每项都标着自己的快捷键）→ 直接按该快捷键选定类型
→ 焦点自动跳到批注框 → 打字 → `Enter` 提交（多行批注用 `Shift+Enter` 换行）。
默认**标完一个就自动退出**拖框模式（要连着标就点开「连续标注」开关）；
下拉框一律不响应滚轮、改值只认显式操作，避免手一滑改掉设置还不知道原值；
重复绑定同一个按键会被设置界面拦下并说明是哪两个动作撞车。
整条链路上的每一步都可重绑定，目标就是"手不离键盘把一话标完"。
...
- **蓝色虚线边界框**：画布上用蓝色虚线框标出当前图层视觉内容的实际范围，
  一眼看清「这一层到底占了哪块地方」；设置 →「显示当前图层的蓝色虚线边界框」
  可关掉（默认开），关闭只影响该框的绘制，定位与缩放行为完全不变；
...
- **后台预加载线程**：打开任务后自动预热当前文件邻域（桌面：后 3 个 + 前 1 个，
  另有前 2 回看松弛，共 6 页；**Android：前 1 + 当前 + 后 1，共 3 页**——激进档
  256MB 装不下 6 页的全图层像素，多预热只会白花解码预算）；
- **两阶段流水线**，状态栏独立显示：
...
- **隐藏图层直接不进结构**：图层列表在解析 PSD 时一次建好（只建结构、不读像素），
  隐藏图层在这一步就被剔除，后续的预热、统计、返修单自然全部忽略它们；
...
- **红框**（粗红镂空、中间透明、不遮原画面），坐标以 **PSD 世界坐标**保存——
  缩放/平移/窗口变化后依然精确跟随原图；
- **21 类预制问题**：居中错误、字体选择错误、字体字重错误、**文字描边粗细错误**、
  **文字颜色错误**、字号错误、文字位置错误、文字间距错误、气泡处理错误、
  原文字擦除错误、背景擦除错误、网点对齐错误、网点残留、修图瑕疵、漏翻、漏字、
  错字、翻译错误、排版错误、文字溢出、其他
  （新增的两类紧跟在其他文字外观类后面、占最顺手的数字键 `4`/`5`）；
- 每类问题有**快捷键**（默认 `1`~`9`、`0`、`Q` `W` `E` `T` `Y` `U` `I` `O` `P` `S` `D`，
  可在设置中重绑定；`R` 留给「红框模式」、`A` 留给「自动框选」）。
  **类型下拉栏里每一项都直接显示自己的快捷键**（`4　文字描边粗细错误`），
  不用记也不用翻设置；
- 两种创建方式：先按快捷键选类型再拖框；或先拖框再选类型（`R` 红框模式）；
- **默认「标完一个就退出」**：无论走 `R` 红框模式还是问题类型快捷键，标完一个
  问题即自动退出拖框模式，光标回到普通浏览状态，避免手一滑又多画一个框；
  想连续标注同类问题，点问题面板里「**连续标注**」开关（**默认关**，勾选后按钮
  高亮为蓝色并显示 `✓`，选择会被记住）——开启后标完一个仍保持待标状态，
  类型快捷键路径还会保持同一类型，可一路标下去，`Esc` 退出；
- **自动框选**（`A` 或问题面板「▣ 自动框选」，**进入拖框模式后可用**）：不必手动拖框，
  直接按当前图层的**视觉内容范围**生成红框——就是画布上那个蓝色虚线框，上下左右
  各外扩 5 像素，中心与虚线框完全一致；整层透明时回退图层 Bounds。生成后走与手动
  拖框完全相同的流程（已选类型就直接建该类型，否则弹类型对话框）；
- **自动框选弹出的类型对话框是"键盘直通"的**：下拉栏自动展开（每一项都标着自己的
  快捷键），直接按该快捷键即为选中，随即自动跳到批注输入框，全程不用碰鼠标；
  若进红框模式前已按过类型快捷键（类型已定），则对话框不再展开下拉栏，直接停在
  批注框。**手动拖框**弹出的对话框保持原逻辑：自己点开下拉栏选择；
- **自定义批注**（`Ctrl+Enter`）直接输入自由文本，如
  「这里应使用 Bold，而不是 Regular」；
- 问题编号与红框徽标一一对应，一份图层可有多个问题（居中错误 + 漏字 + …）；
- **红框显示范围**（设置 →「问题红框显示」，默认**整页**）：默认始终显示
  **当前 PSD 的全部问题**——跨图层，翻到哪页就看全哪页标注，不必逐个图层点过去；
  可切换为**仅当前图层**（旧版行为，只显示当前图层红框）。切换即时生效，
  不改变任何已保存的问题数据；
- **编号检查/重排**（工具栏「检查问题编号」）：编号在新增问题时取「最大值 + 1」，
  删除问题、或监制完后面的 PSD 又回头补问题时会出现空号、跳号，红框徽标与问题
  面板的 `#编号` 看起来就不对。点该按钮即按 **PSD 顺序 → 图层顺序 → 创建顺序**
  重排为 1..N 并立即保存（归属 PSD/图层已不存在的问题保留、排在最后并提示）；
  重排只在按钮触发时执行——新增/删除问题时不做编号维护，标记路径零额外开销；
  扫描在后台线程进行并显示进度框，大任务不卡界面，可随时取消。
...
- **随时收起当前任务**：「文件 → 关闭当前任务」(`Ctrl+W`) 回到未打开状态——
  关闭前会先同步落盘（标完最后一项立刻按也不丢改动），任务文件保留在磁盘上，
  重新打开同一文件夹或走「最近打开」即可接着标；保存失败（磁盘满 / 只读 / 权限）
  会**中止关闭**并说明原因，不静默丢进度。关闭后画布中央会提示下一步入口
  （打开 PSD / 文件夹的当前快捷键），画布、面板、缓存全部释放；
...
- 封面：任务名、生成时间、PSD 数、总图层、通过/未通过/未监制；未完成时明确标注
  「任务状态：未完成」；
- **目录 + 书签**：有问题明细时自动插入目录页（PSD 条目 + 图层子条目、
  点线页码、**条目可点击跳转**），同时写入 **PDF 书签（阅读器侧栏大纲）**；
  页码在排版时按**实际落页**回填并迭代到稳定（明细清单自动换行/换页也不会错位），
  点目录里的 PSD 就能精确跳到它真正所在的那一页；
- PSD 总览表：每页的进度与问题数；默认**隐藏「全部通过且无问题」的页**
  （表下注明隐藏数量，报告更聚焦），未通过 / 未监制的页始终保留；
  生成对话框里可勾选/取消（**默认勾选，每次生成都由用户确认**），
  设置中也可预设，选择会被记住；
- 问题明细页：**页面图像 + PDF 矢量红框 + ①②③ 编号 + 批注**（红框由 PDF 矢量
  矩形绘制，不截图 GUI）；
- **一页 PSD 一页明细**：同一 PSD 的全部问题共用一张页面图像、合并到同一页，
  下方按图层分组列出问题（编号页内连续）；不再一个图层/一个问题占一页，
  同一张图不重复嵌入；
- 全篇使用与界面同一份 **MiSans** 字体（内嵌子集）；编号超出字体覆盖范围
  （①～⑩ 之外）自动改用 (11) 写法，不会出现空白方块；
- **页面图像可选压缩**：PNG 无损（默认）或 JPEG（质量 60～95%，可选），
  JPEG 下报告体积显著变小、画质略降；红框与编号始终是 PDF 矢量，不受影响。
  生成对话框里可直接改选（选择会被记住），设置 →「MangaProof 返修单」也可预设；
- 自动命名兼容嵌字脚本固定路径：所在文件夹为 `output` 时默认取上一级文件夹名；
- 未全部完成时也允许生成，返修单会如实标注未完成；
- **完成监制自动生成**（设置 →「完成监制后自动生成返修单」，**默认开**）：
  所有图层检查完毕时自动生成一份（沿用已保存的名称 / 图片格式 / 总览选项），
  同一次完成只生成一次，不会连按 Enter 反复生成；之后再补加或修改问题再完成
  时会重新生成一遍。不想自动生成可在设置里关掉，`Ctrl+R` 随时手动生成；
- **后台生成 + 进度框**：提取页面图像、编码、写 PDF 全在后台线程跑，
  进度框逐页推进（准备 → 封面与总览 → 问题明细页 i/N → 写入 PDF），
  大批量任务不再冻结界面；点「取消」在页边界中断，不会留下半成品文件。
...
- **只认 PSD 里可见的图层**：在 Photoshop 中隐藏的图层（包括位于隐藏的组 /
  文件夹里的图层）不进图层列表、不计入任何统计与进度、不参与预加载、也不会
  出现在返修单里——隐藏 = 本轮不检查，省内存也省时间；把它们重新显示出来，
  下次打开任务即自动回到列表（先前对同名图层做过的监制记录仍在任务文件里）。

### 9. 最近打开：一键回到上次的任务

- 「文件 → 最近打开」列出最近 **10** 个打开过的文件夹 / 单个 PSD，**最近的排最前**；
  重复打开只提前、不新增（同一路径的不同写法——尾斜杠、`..`、软链接——归一化后合并）；
- 记录独立存在 **程序目录/recent.json**，与 `settings.json` 彻底分开：设置被重置、
  损坏或被整份重建都不会连带清掉打开历史（旧版本混在 settings.json 里的记录
  首次启动自动迁移过来，之后不再写回设置文件）；
- 路径暂时不可用（网络盘 / 移动盘未挂载）时记录**保留**，点它才提示，并顺手把这条
  失效记录移除——不会每点一次弹一次同样的框；
- 菜单底部「清除最近打开记录」一键清空；
- 该功能**不占用快捷键**（无需绑定），只用菜单操作。
...
> **第一次使用**（程序目录下还没有 `settings.json`）：启动后会直接把**设置页面**
> 打开一次，让你按自己的习惯过一遍——不改任何一项也完全没问题，默认值已在代码里
> 定好，随时可在 设置 →「设置…」里再调。主窗口顶部此时还有一条**可关闭的提醒条**
> （`打开设置…` / `✕`），点确定保存或正常退出过一次后就不再出现。
...
| 自动框选当前图层 | `A`（需先进入拖框模式） |
| 自定义批注 | `Ctrl+Enter` |
| 批注框内：确认 / 换行 | `Enter` / `Shift+Enter`（对话框内有提示） |
| 问题类型 | `1`~`9`、`0`、`Q` `W` `E` `T` `Y` `U` `I` `O` `P` `S` `D`（预制 21 类，可配置；下拉栏直接显示各自快捷键） |
| 打开 PSD / 文件夹 | `Ctrl+O` / `Ctrl+Shift+O` |
| 关闭当前任务 | `Ctrl+W`（先自动保存再关闭；无任务时该按钮为灰） |
| 生成返修单 | `Ctrl+R` |
| 最近打开 | 仅菜单（文件 → 最近打开），**不绑定快捷键** |

> 界面上的下拉框（显示比例、问题类型、返修单选项、设置项…）**一律不响应鼠标滚轮**：
> 滚轮只用于滚动/缩放画面，改下拉框值请点击展开或用键盘 `↑`/`↓`——
> 避免"想滚页面却顺手改掉了设置，还不知道原来选的是什么"。

> **快捷键冲突保护**：同一个按键被绑给两个动作时，Qt 会判定为「歧义」而
> **两个都不触发**（表现就是按下毫无反应）。设置 →「设置快捷键…」会实时列出
> 冲突并拒绝保存这种配置；真按下冲突键时状态栏会直接说明是哪两个动作撞车。
> 旧版本默认把「漏字」也绑在 `R`（与「红框模式」撞车）导致 `R` 完全没反应，
> 升级后会自动把「漏字」挪到 `P`（用户已自行改绑过的配置不动）。

## 数据文件（三层隔离）

```
程序级设置   程序目录/settings.json          （显示比例、红框显示范围、蓝色虚线框开关、连续标注开关、对比模式与速度、快捷键、PDF 开关与报告选项、内存策略…）
程序级状态   程序目录/recent.json            （最近打开记录；与设置分开存，重置设置不清历史）
...
- 返修单 PDF 同样在后台线程生成（模态进度框逐页推进，可取消），大任务不冻结界面；
- 窗口外文档大图自动回收，内存有界；
- **内存回收策略三档**（设置中热切换，无需重启）：
  ...
  - 宽松 / 平衡 / 激进 = LRU 768 / 512 / 256MB，bg 预生成池 768 / 512MB /
    仅 2 张；当前页背景图钉住，不被淘汰；
  - **Android 端固定为「激进」档**：读取设置时即强制、并把值写回 `settings.json`，
    设置页该档灰显不可改（移动设备内存紧张，档位不交给用户选）；桌面端仍是三档。
...
- 字体：统一使用小米 **MiSans**（`font/MiSans-Medium.ttf`），直接运行与打包路径
  自动识别；界面与**返修单 PDF 用同一份字体文件**（PDF 内嵌 MiSans 子集，
  按用到的字符裁剪，报告体积只增加约 10KB；字体缺失时回退内置宋体，
  生成不受影响）；依据《MiSans 字体知识产权许可协议》使用（「关于」与
  README 注明、不改编、不单独分发），缺失时回退系统字体。
...
mangaproof/
├── main.py            # 入口
├── ui/                # 主窗口、Viewer、面板、设置/许可对话框、预加载/返修单/编号线程
├── psd/               # PSD 加载、文档模型、LRU 图像缓存
├── camera/            # Camera、视觉中心、自动缩放
├── review/            # 监制状态、问题、导航、编号检查/重排、持久化 + 哈希验证
